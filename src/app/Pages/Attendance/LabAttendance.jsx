...
function LabAttendance({ setLoading, collegeId }) {
  var curr = new Date();
  ...
  const [defaultAttendance, setDefaultAttendance] = useState(null);
  ...
  const getDataAllData = async () => {
    ...
    const [data1, data2, data3, data4] = await Promise.all([
      ...
      axios({
        ...config,
        url: ACADEMICS_ADD_SUBJECT + `?college_id=${collegeId}&type=PRACTICAL`,
      })
        .then((res) => {
  ...
  const BatchData = async () => {
    const config = {
      method: "get",
      url: `${ACADEMICS_GET_BATCH}?class_id=${user?.class_id}&status=ACTIVE`,
    };

    await axios(config)
      .then((res) => {
        setBatchOpt(res.data.data);
      })
      .catch((err) => {
        console.log(err);
      });
  ...
  const getData = async () => {
    ...
    await axios({
      ...
      .then((res) => {
        ...
        setData(res.data.data?.filter((s) => s.course_type == "PRACTICAL"));
        setVisibility(false);
      })
      .catch((err) => {
        console.log(err);
    ...
    await axios({
      ...config,
      url: `${ACADEMICS_GET_ASSIGN_STU}?class_id=${user?.class_id}&session_id=${user?.session_id}&batch_id=${user?.batch_id}`,
    })
      .then((res) => {
        y2 = res.data.data;
        console.log('y2 - ', res.data.data);
      })
      .catch((err) => {
        console.log(err);
        toast.error("Something went wrong");
    ...
    try {
      await axios({
        ...
        url:
          STUDENT_SESSION +
          `?section_id=${user?.section_id}&class_id=${user?.class_id}&session_id=${user?.session_id}&status=ACTIVE&batch_id=${user?.batch_id}`,
      })
        .then(async (res) => {
          ...
          for (let i of y2) {
            studentIdsObj[i.user_id] = true;
          }

          let commonElements = [];

          for (let item of res.data.data) {
            if (studentIdsObj[item.user_id]) {
              commonElements.push(item);
            }
          }
          console.log("Ans - ", commonElements);
          setStudentData(commonElements);
        })
        .catch((err) => {
          console.log(err);
        });
  ...
  const HandleAttendenceSubmit = async () => {
    ...
    Swal.fire({
      ...
      console.log(s.isConfirmed);
      await axios(config)
        .then(() => {
          toast.success("Added");
          getAttendenceData(xy, date);
        })
        .catch((err) => {
          toast.error("something went wrong");
        });
  ...
  const updateAtt = async () => {
    ...
    Swal.fire({
      title: "Do you want to update the attendence",
      showCancelButton: true,
      confirmButtonText: "Save",
    }).then(async (s) => {
      if (s.isConfirmed) setLoading(1);
  ...
  const getAttendenceData = async (atData, d) => {
    setLoading(1);
    ...
    await axios(config)
      .then((res) => {
        if (
          ...
          setAbsentArray(res?.data?.data?.absent_students);
          setEdit1(false);
          setPresentArray(res?.data?.data?.present_students);
            ...
            <div className="row">
              <div className="col-12">
                <div className="page-title-box d-flex align-items-center justify-content-between">
                  <h4 className="mb-0">Lab Attendance</h4>
