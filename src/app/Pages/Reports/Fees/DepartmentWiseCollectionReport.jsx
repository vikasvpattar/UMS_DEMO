...
import { REPORT_FEE_DETAILS , PAYMENT_DEPARTMENT_WISE_SQL} from "../../../utils/Reports.apiConst";
...
const ProgramWiseCollectionReport = ({ setLoading, collegeId }) => {
  ...
  const getPaymentData = async () => {
    ...
    const config = {
      method: "get",
      url: `${PAYMENT_DEPARTMENT_WISE_SQL}?transaction_date=${user?.date}&to_transaction_date=${user?.to_transaction_date}&department_id=${department}&college_id=${collegeId}`,
      headers: {
        Authorization: `Bearer ${sessionStorage.getItem("UMS_auth")}`,
        "Content-Type": "application/json",
      },
    ...
    await axios(config)
        ...
        for(let j in transactionsArray[i].transactions) {
          ...
          for(let k in paymentDetails) {
            if(transactionsArray[i].transaction_id == paymentDetails[k]?.payment_id) {
              ...
              transactionsArray[i].transaction_date = transactionsArray[i]?.transactions[j]?.transaction_date;
            }
          }
        }
      }
      ...
      let deptData = Object.keys(deptTransactions).map(dept_id => ({
      dept_id: dept_id,
      ...
      let finalData = [];
      for(let i in deptData) {
          const dateTransactions = deptData[i]?.transactions?.reduce((acc, curr) => {
          ...
          const temp = Object.keys(dateTransactions).map(transaction_date => ({
            date: transaction_date,
            transactions: dateTransactions[transaction_date]
        }));

        deptData[i].transactions = temp;
      }

      console.log('final data = ', deptData);
                  ...
                  <div className="col-md-4">
                    <div className="form-group">
                      ...
                      >
                        ...
                        {departmentData?.filter((s) => s.college_id == collegeId)?.map((i, key) => (
