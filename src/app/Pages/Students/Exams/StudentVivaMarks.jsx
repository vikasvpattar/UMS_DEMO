...
import { STUDENT_VIVA } from "../../../utils/Examination.apiConst";
import { LOCAL_DEPARTMENT } from "../../../utils/LocalStorageConstants";

function StudentsVivaMarks({ setLoading, collegeId }) {
  ...
  const handleChange = (id, name, value) => {
    ...
    for (const el in mainData) {
      if (mainData[el]?.student_id == id) {
        ...
        // if(
        //   name === "marks" &&
        //   (!subjectOpt?.find((s) => s?.id == subSelected)?.viva_max_marks ||
        //    !subjectOpt?.find((s) => s?.id == subSelected)?.viva_min_marks)
        // ){
        //   toast.error("Please Enter Viva Max and Min Marks in Add Subject");
        //   return;
        // }
        ...
        if (
          name === "marks" &&
          subjectOpt?.find((s) => s?.id == subSelected)?.viva_max_marks &&
          value > subjectOpt?.find((s) => s?.id == subSelected)?.viva_max_marks
        ) {
          toast.error(
            `Marks cannot be greater than viva max marks (${
              subjectOpt?.find((s) => s?.id == subSelected)?.viva_max_marks
            })`
          );
          return; // Return if validation fails
  ...
  const getData = async () => {
    ...
    const config = {
      method: "get",
      url: `${STUDENT_VIVA}?session_id=${user?.session_id}&course_id=${user?.course_id}`,
      headers: {
  ...
  const handleSubmit = async () => {
    ...
    mainData.viva_min_marks = subjectOpt?.filter(
      (s) => s.id == subSelected
    )[0]?.viva_min_marks;
    mainData.viva_max_marks = subjectOpt?.filter(
      (s) => s.id == subSelected
    )[0]?.viva_max_marks;
    setLoading(1);
    const config = {
      method: "post",
      url: `${STUDENT_VIVA}/all`,
      headers: {
        Authorization: `Bearer ${sessionStorage.getItem("UMS_auth")}`,
        "Content-Type": "application/json",
      },
              ...
              <div className="col-12">
                ...
                <div className="row mt-3">
                  <div className="card col-12 p-2">
                    <div className="card-title">Student Viva Marks</div>
...
export default StudentsVivaMarks;
