...
import { INTERNALS } from "../../../utils/Examination.apiConst";
import { LOCAL_DEPARTMENT } from "../../../utils/LocalStorageConstants";

function StudentsExams({ setLoading, collegeId }) {
  ...
  const handleChange = (id, name, value) => {
    console.log(id, name, value, subjectOpt);

    const arr = [];

    const maxMarks = subjectOpt?.find(
      (s) => s?.id == subSelected
    )?.internal_max_marks;
    const minMarks = subjectOpt?.find(
      (s) => s?.id == subSelected
    )?.internal_min_marks;
    ...
    for (const el in mainData) {
      if (mainData[el]?.student_id == id) {
        ...
        if (name === "marks" && mainData[el]?.absent) {
          ...
          toast.error("This Student is Absent");
          return; // Return if validation fails
        }
        ...
        //   name === "marks" &&
        //   (
        //     !subjectOpt?.find((s) => s?.id == subSelected)?.internal_max_marks ||
        //     !subjectOpt?.find((s) => s?.id == subSelected)?.internal_min_marks ||
        //     subjectOpt?.find((s) => s?.id == subSelected)?.internal_max_marks === 0 ||
        //     subjectOpt?.find((s) => s?.id == subSelected)?.internal_min_marks === 0
        //   )
        // ) {
        //   toast.error("Please Add internal Max and Min marks in Add Subject");
        //   return; // Return if validation fails if its empty and also value is 0(zero)
        // }
        ...
        //     !subjectOpt?.find((s) => s?.id == subSelected)?.internal_min_marks)
        // ) {
        //   // toast.error("Max and Min marks are required to enter marks");
        //   toast.error("Please Add internal Max and Min marks in Add Subject");
        //   return; // Return if validation fails
        // }
        ...
        if (
          ...
          toast.error("Please Add internal Max and Min marks in Add Subject");
          return; // Return if validation fails when its empty
        }
        ...
        //   subjectOpt?.find((s) => s?.id == subSelected)?.internal_max_marks &&
        //   subjectOpt?.find((s) => s?.id == subSelected)?.internal_min_marks &&
        //   value > subjectOpt?.find((s) => s?.id == subSelected)?.internal_max_marks
        // ) {
        //   toast.error(
        //     `Marks cannot be greater than internal max marks (${subjectOpt?.find(
        //       (s) => s?.id == subSelected
        //     )?.internal_max_marks})`
        //   );
        //   return; // Return if validation fails
  ...
  const getData = async () => {
    ...
    const config = {
      method: "get",
      url: `${INTERNALS}?session_id=${user?.session_id}&course_id=${user?.course_id}&status=ACTIVE`,
      headers: {
  ...
  const getStudentsData = async () => {
    ...
    const config = {
      method: "get",
      url: `${STUDENT_SESSION}?college_id=${collegeId}&session_id=${user?.session_id}&class_id=${user?.class_id}&semester_id=${user?.semester_id}&status_student=ACTIVE&status=ACTIVE`,
      headers: {
        Authorization: `Bearer ${sessionStorage.getItem("UMS_auth")}`,
        "Content-Type": "application/json",
      },
  ...
  const getSubjects = async () => {
    ...
    const config = {
      method: "get",
      url: `${ACADEMICS_ADD_SUBJECT}?class_id=${user?.class_id}&semester_id=${user?.semester_id}&department_id=${user?.department_id}&getInactive=0`,
      headers: {
        Authorization: `Bearer ${sessionStorage.getItem("UMS_auth")}`,
        "Content-Type": "application/json",
      },
    ...
    await axios(config)
      .then((res) => {
        console.log("Subjects Here", res.data.data);
  ...
  const handleSubmit = async () => {
    ...
    for (const i of mainData) {
      i.max_marks = subjectOpt?.find(
        (s) => s?.id == subSelected
      )?.internal_max_marks;
      i.min_marks = subjectOpt?.find(
        (s) => s?.id == subSelected
      )?.internal_min_marks;
    }
          ...
          <div className="modal-content ">
            ...
            <div className="modal-body">
              <div className="row">
                ...
                {/* <div className="col-md-4">
                  <div className="form-group">
                    <label htmlFor="">Internal Max Marks</label>
                    <input
                      type="text"
                      value={
                        studentData?.find(
                          (s) => s?.data?.student_id == editData?.student_id
                        )?.studentInfo?.internal_max_marks
                      }
                      className="form-control"
                      readOnly={true}
                      disabled={true}
              ...
              <div className="col-12">
                ...
                <div className="row mt-3">
                  <div className="card col-12 p-2">
                    <div className="card-title">Student Internal Marks</div>
