...
function Review({ type, empId, data, reloadData, setLoading, entitlement }) {

    const [balance, setBalance] = useState(0)

    const [employeeId, setEmployeeId] = useState(sessionStorage.getItem(SESSION_EMPLOYEE_ID)) 
    ...
    const Difference_In_Time = new Date(user.to_date).getTime() - new Date(user.from_date).getTime();
    ...
    const calculateDifference = (a1,a2,e) => {
        ...
        if(Difference_In_Days+1<0) return toast.error("To date cannot be less then From date")
        if(Difference_In_Days+1>balance) return toast.error("Number of days cannot be Greater then balance Days")
        handleChange(e);
        setUser(prevValue => ({
            ...prevValue,
                                ...
                                <div className="col-12">
                                    <div className="form-group">
                                        ...
                                        <select
                                            ...
                                            onChange={(e) => { handleChange(e); checkBalance(e.target.value) }}
                                        >
                                            <option value="">Select Entitlement</option>
                                            {
                                                entitlement?.map((i, key) => (
                                                    <option value={i.id}>{i.leave_type} , {i.entitled_days}</option>
                                                ))
                                            }
                                        </select>
                                    </div>
                                ...
                                <div className="col-md-6">
                                    <div className="form-group">
                                        ...
                                        <input
                                            ...
                                            onChange={(e)=>{calculateDifference(e.target.value,user.to_date,e)}}
                                        />
                                    </div>
                                </div>
                                <div className="col-md-6">
                                    <div className="form-group">
                                        ...
                                        <input
                                            ...
                                            name='to_date'
                                            value={user.to_date}
                                            onChange={(e)=>{calculateDifference(user.from_date,e.target.value,e)}}
                                        />
                                    </div>
                                </div>
                                <div className="col-12">
