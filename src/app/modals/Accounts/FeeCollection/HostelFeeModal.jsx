...
const HostelModalFee = ({ setLoading, reloadData, maxAmount }) => {
    ...
    const [data1] = await Promise.all([
      axios({
        ...
        url: `${ADDHOSTELFEE}?session_id=${
          ...
          "department_id"
        )}&student_id=${searchParams.get("student_session_id")}&year=${searchParams.get("year")}`,
      })
        .then((res) => {
          setLoading(0);
  ...
  const handleSubmit = async () => {
    ...
    console.log('data to backend - ', obj);

    console.log('url - ', `${ADDHOSTELFEE}/${feeid}`)
    const config1 = {
      method: "put",
